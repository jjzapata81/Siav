...
public class PagosBean {
	...
	public void guardarArchivo(String nombreArchivo, String codigoCuenta, String usuario, File archivoPagos) {
		...
		try {
			...
			String directorio = getDirectorio(pagos.get(0).getFecha());
	...
	private Pago convert(String linea){
		try{
			...
			Pago pago = new Pago();
			pago.setFecha(setFecha(atributos[0].trim(), "dd-MM-yyyy"));
			pago.setValor(atributos[5].trim());
			pago.setNumeroFactura(getValorAuxiliar(atributos[8], "-", 0));
			return pago;
		}catch(Exception e){
	...
	private String getValorAuxiliar(String valor, String separador, int posicion) {
		return valor.split(separador)[posicion].trim();
	}
	...
	private String getDirectorio(Date fecha) {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyyMM");
		return fecha == null ? "SINFECHA" : formatter.format(fecha);
	}
