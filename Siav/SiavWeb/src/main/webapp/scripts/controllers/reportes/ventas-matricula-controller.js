...
define(['siav-module', 'reportes-services', 'modal-email', 'modal-factory', 'constantes'], function (app) {
    ...
    return app.controller('ventas-matricula-controller', ['$scope', '$sce', 'reportesServices', 'modalEmail', 'modalFactory', 'CONSTANTES',
	  function($scope, $sce, reportesServices, modalEmail, modalFactory, CONSTANTES){
    	...
    	$scope.onDescargar = function(){
    		if($scope.validar()){
    			reportesServices
        		.descargar(REPORTE, "ventas_ciclo" + $scope.filtro.ciclo + ".xlsx", $scope.filtro);
    		}
    	}
    	...
    	$scope.onBuscarPDF = function(){
    		if($scope.validar()){
    			reportesServices
        		.descargarPDF(REPORTE, "ventas_ciclo" + $scope.filtro.ciclo + ".pdf", $scope.filtro);
    		}
    	}
